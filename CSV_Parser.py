...
def csvParser (originalFile: str, inputFile: str, timestamp: str) -> str:
    ...
    for x in range(0, len(lookupValues)):
        with open(originalFile, encoding='utf-16', errors='ignore') as ipf:
            for i, line in enumerate(ipf):
                if line.find(lookupValues[x]) != -1:
                    list1.append(lookupValues[x].replace('_',' ').title().replace(' ','') + ',')
                    list2.append(i+1)
    ...
    for y in range(0, len(list2)):
        with open(originalFile, encoding='utf-16', errors='ignore') as ipf:
            for i, line in enumerate(ipf):
                if i == list2[y]:
                    list2[y] = line.strip().replace(',','.')

    for z in range(0, len(list1)):
        if z < len(list1)-1:
            out3 = out3 + list1[z] + list2[z] + '\n'
        else:
            out3 = out3 + list1[z] + list2[z]
